...
async def collect_urls_handler(message: types.Message, state: FSMContext):
    ...
    if not urls:
        await message.answer("Пожалуйста, отправьте ссылки, разделенные запятыми. Пример:\n⠀https://youtu.be/xyz, ⠀https://vk.com/video/12345⠀")
        return
...
async def process_next_url(message: types.Message, state: FSMContext):
    ...
    try:
        ...
        if link_type == "YouTube":
            file_path, title = await download_video_with_quality(current_url, {'format_id': 'best'}, message.from_user.id)
...
async def handle_text(message: types.Message, state: FSMContext):
    ...
    if text == "отправить ссылку 🔗":
        ...
        await message.answer("Отправьте ссылки через запятую. \n\nПример:\nhttps://youtu.be/xyz, https://rutube.ru/video/idk")
