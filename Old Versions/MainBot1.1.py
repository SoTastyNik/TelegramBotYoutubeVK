...
async def process_url(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if link_type == "YouTube":
        ...
        await update.message.reply_text("Неподдерживаемый URL. Пожалуйста, отправьте ссылку на YouTube, VK Клип, или VK Историю.")
        return START
...
async def handle_action_selection(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if action == 'конвертировать в mp3':
        ...
        if not formats:
            await update.message.reply_text("Не удалось получить информацию о видеоформатах.", reply_markup=markup_start)
            return PROCESS
        ...
        await update.message.reply_text("Загрузка VK Истории началась...")
        vk_story_path = await asyncio.to_thread(download_vk_history, url, update.message.from_user.id)
        await send_file(update, vk_story_path, " ", 'video')
        return PROCESS
...
def download_vk_history(url, user_id):
    ...
    for i in res.json()['response']['items'][0]['video']['files'].keys():
        if 'mp4' in i:
            quality[i.split('_')[1]] = res.json()['response']['items'][0]['video']['files'][i]
    file_type = req_1['type']
    if file_type == 'video':
        req = requests.get(quality['480'])

        with open(f"{user_id}_video.mp4", 'wb') as x:
            x.write(req.content)
    return f"{user_id}_video.mp4"

# Универсальная функция для скачивания VK контента (Клипов и Историй)
